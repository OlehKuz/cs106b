...
void openStickerImage(GBufferedImage &stickerImg);
void getLocation(int &startR, int &startC);
...
void greenScreen(Grid<int> &img){
    ...
    openStickerImage(stickerImg);
    Grid<int> stickerImage = stickerImg.toGrid();
    cout<<"click on image"<<endl;

    int tresholdGreen = getInteger("How much tolerance should "
                                   "we have for green that isn’t pure green");

    int startRow;
    int startCol;
    getLocation(startRow,startCol);


    for(int row = 0; row < stickerImage.height(); row++) {
        for(int col = 0; col < stickerImage.width(); col++) {
            int stickerPixel = stickerImage[row][col];
            int red, green, blue;
            GBufferedImage::getRedGreenBlue(stickerPixel, red, green, blue);

            if(stickerPixel-GREEN>tresholdGreen){
                if(img.inBounds(row+startRow,col+startCol)){
                    img[row+startRow][col+startCol]=stickerPixel;
                }
...
void getLocation(int &startR, int &startC){
    getMouseClickLocation(startR,startC);
    /*while(!(startR>=0&startC>=0)){
        startR = getInteger("Enter location to place image (row) :");
        startC = getInteger("Enter location to place image (column) :");
    } */
}
...
void openStickerImage(GBufferedImage &stickerImg){
    ...
    while(!successOpenImage) {
        file = getLine("Enter name of sticker image file to open: ");
        successOpenImage = openImageFromFilename(stickerImg, file);
    }
}
